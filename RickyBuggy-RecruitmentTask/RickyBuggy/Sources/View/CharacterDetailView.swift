    ...
    var photoSection: some View {
        VStack(alignment: .center, spacing: 8) {
            CharacterPhoto(data: viewModel.CharacterPhotoData)
                .aspectRatio(contentMode: .fit) // Change contentMode to .fit
                .frame(maxWidth: UIScreen.main.bounds.width * 0.8, alignment: .center) // constrain width
                .clipShape(RoundedRectangle(cornerRadius: 5)) // Use clipShape instead of cornerRadius
        }
        .padding()
    }
}
